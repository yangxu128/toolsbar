...
import { Home, ChevronRight, Star, Radio, Signal, Building2, TowerControl, BookOpen } from 'lucide-react'
import { useFavStore } from '@/lib/fav-store'
import {
  LTE_BANDS, NR_BANDS, calcLteFreq, calcLteEarfcn, calcNrFreq, calcNrArfcn,
  decodeECI, encodeECI, decodeNCI, encodeNCI, type LteBand, type NrBand
              ...
              return (
                <button key={tab.key} onClick={() => setActiveTab(tab.key)}
                  className={`flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium border transition-all ${
                    active ? 'bg-[hsl(var(--primary))] text-white border-[hsl(var(--primary))]' : 'bg-[hsl(var(--card))] text-[hsl(var(--muted-foreground))] border-[hsl(var(--border))] hover:border-[hsl(var(--primary))] hover:text-[hsl(var(--foreground))]'
                  }`}>
...
function ResultBox({ children, empty = false }: { children: React.ReactNode; empty?: boolean }) {
  return (
    <div className={`mt-4 p-4 rounded-xl border-l-4 ${empty ? 'border-l-[hsl(var(--border))] bg-[hsl(var(--muted))] text-[hsl(var(--muted-foreground))] italic' : 'border-l-[hsl(var(--primary))] bg-[hsl(var(--muted))]'}`}>
      {children}
    </div>
  )
}
...
function Badge({ children, color = 'blue' }: { children: React.ReactNode; color?: 'blue' | 'green' | 'amber' | 'slate' | 'violet' }) {
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
      <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
        <h3 className="text-base font-semibold text-blue-600 mb-4 flex items-center gap-2"><Signal className="w-4 h-4" />EARFCN → 频率</h3>
        <div className="mb-3">
          ...
          <input type="number" min={0} max={65535} value={earfcn} onChange={e => setEarfcn(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleCalcFreq()}
            className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"
            placeholder="例如：1300" />
        </div>
        <button onClick={handleCalcFreq} className="w-full py-2.5 bg-[hsl(var(--primary))] text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">计算频率与频段</button>
        {freqResult ? (
          freqResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{freqResult.error}</span>} /></ResultBox> : (
            <ResultBox>
              <ResultRow label="频段 Band" value={<><Badge color="amber">Band {freqResult.band.band}</Badge> <Badge color={freqResult.band.mode === 'FDD' ? 'blue' : 'green'}>{freqResult.band.mode}</Badge></>} />
      ...
      <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
        <h3 className="text-base font-semibold text-blue-600 mb-4 flex items-center gap-2"><Signal className="w-4 h-4" />频率 → EARFCN</h3>
        <div className="mb-3">
          <label className="block text-xs font-medium text-[hsl(var(--muted-foreground))] mb-1">选择频段</label>
          <select value={bandSel} onChange={e => setBandSel(e.target.value)}
            className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
        ...
        <button onClick={handleCalcEarfcn} className="w-full py-2.5 bg-[hsl(var(--primary))] text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">计算 EARFCN</button>
        {earfcnResult ? (
          earfcnResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{earfcnResult.error}</span>} /></ResultBox> : (
            <ResultBox>
              <ResultRow label="频段 Band" value={<><Badge color="amber">Band {earfcnResult.band.band}</Badge> <Badge color={earfcnResult.band.mode === 'FDD' ? 'blue' : 'green'}>{earfcnResult.band.mode}</Badge></>} />
              ...
              <ResultRow label="EARFCN" value={<span className="text-lg text-[hsl(var(--primary))]">{earfcnResult.earfcn}</span>} />
...
function NrTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          ...
          <div className="mb-3">
            ...
            <input type="number" min={0} max={3279165} value={nrArfcn} onChange={e => setNrArfcn(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleNrFreq()}
              className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"
              placeholder="例如：620000" />
          </div>
          <button onClick={handleNrFreq} className="w-full py-2.5 bg-violet-600 text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">计算频率</button>
          {nrFreqResult ? (
            nrFreqResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{nrFreqResult.error}</span>} /></ResultBox> : (
              <ResultBox>
                ...
                <ResultRow label="匹配频段" value={nrFreqResult.matchedBand ? <><Badge color="violet">{nrFreqResult.matchedBand.band}</Badge> <Badge color={nrFreqResult.matchedBand.mode === 'FDD' ? 'blue' : 'green'}>{nrFreqResult.matchedBand.mode}</Badge></> : '未匹配到已知频段'} />
              </ResultBox>
            )
        ...
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          ...
          <div className="mb-3">
            ...
            <select value={nrBandSel} onChange={e => setNrBandSel(e.target.value)}
              className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
              <option value="">-- 选择频段 --</option>
          ...
          <div className="mb-3">
            ...
            <input type="number" step={0.001} value={nrFreqInput} onChange={e => setNrFreqInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleNrArfcn()}
              className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"
              placeholder="例如：3410.400" />
          </div>
          <button onClick={handleNrArfcn} className="w-full py-2.5 bg-violet-600 text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">计算 NR-ARFCN</button>
          {nrArfcnResult ? (
            nrArfcnResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{nrArfcnResult.error}</span>} /></ResultBox> : (
              <ResultBox>
                <ResultRow label="频段" value={<><Badge color="violet">{nrArfcnResult.band.band}</Badge> <Badge color={nrArfcnResult.band.mode === 'FDD' ? 'blue' : 'green'}>{nrArfcnResult.band.mode}</Badge></>} />
      ...
      <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
        <strong>NR-ARFCN 公式（3GPP TS 38.101）：</strong><br />
...
function EciTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          <h3 className="text-base font-semibold text-blue-600 mb-4 flex items-center gap-2"><Building2 className="w-4 h-4" />ECI → eNB ID + Cell ID</h3>
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              <input type="text" value={eciInput} onChange={e => setEciInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleDecode()}
                className="flex-1 px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"
                placeholder="例如：108830209 或 0x067C4001" />
              <select value={eciBase} onChange={e => setEciBase(e.target.value)}
                className="w-[100px] px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
                <option value="10">十进制</option>
                <option value="16">十六进制</option>
              </select>
            </div>
          </div>
          <button onClick={handleDecode} className="w-full py-2.5 bg-[hsl(var(--primary))] text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">解析 ECI</button>
          {decodeResult ? (
            decodeResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{decodeResult.error}</span>} /></ResultBox> : (
              <ResultBox>
                ...
                <ResultRow label="eNB ID" value={<span className="text-blue-600">{decodeResult.enbId}</span>} />
                <ResultRow label="eNB ID (Hex)" value={`0x${decodeResult.enbId.toString(16).toUpperCase().padStart(5, '0')}`} />
                <ResultRow label="Cell ID" value={<span className="text-blue-600">{decodeResult.cellId}</span>} />
        ...
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          <h3 className="text-base font-semibold text-blue-600 mb-4 flex items-center gap-2"><Building2 className="w-4 h-4" />eNB ID + Cell ID → ECI</h3>
          <div className="mb-3">
          ...
          <button onClick={handleEncode} className="w-full py-2.5 bg-[hsl(var(--primary))] text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">合成 ECI</button>
          {encodeResult ? (
            encodeResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{encodeResult.error}</span>} /></ResultBox> : (
              <ResultBox>
                <ResultRow label="ECI (十进制)" value={<span className="text-lg text-blue-600">{encodeResult.eci}</span>} />
      ...
      <div className="p-4 rounded-xl bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-xs text-blue-800 dark:text-blue-300 leading-relaxed">
        <strong>ECI 结构（3GPP TS 36.413）：</strong><br />
...
function NciTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          ...
          <div className="mb-3">
            ...
            <div className="flex gap-2">
              <input type="text" value={nciInput} onChange={e => setNciInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleDecode()}
                className="flex-1 px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))] placeholder:text-[hsl(var(--muted-foreground))]"
                placeholder="例如：68719476735 或 0xFFFFFFFFF" />
              <select value={nciBase} onChange={e => setNciBase(e.target.value)}
                className="w-[100px] px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
                <option value="10">十进制</option>
                <option value="16">十六进制</option>
              </select>
          ...
          <div className="mb-3">
            ...
            <select value={gnbIdLen} onChange={e => setGnbIdLen(Number(e.target.value))}
              className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
          ...
          <button onClick={handleDecode} className="w-full py-2.5 bg-violet-600 text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">解析 NCI</button>
          {decodeResult ? (
            decodeResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{decodeResult.error}</span>} /></ResultBox> : (
              <ResultBox>
        ...
        <div className="bg-[hsl(var(--card))] rounded-2xl border border-[hsl(var(--border))] p-6">
          ...
          <div className="mb-3">
            ...
            <select value={gnbIdLenEnc} onChange={e => setGnbIdLenEnc(Number(e.target.value))}
              className="w-full px-3 py-2.5 rounded-lg border border-[hsl(var(--border))] text-sm outline-none focus:border-[hsl(var(--primary))] bg-[hsl(var(--card))] text-[hsl(var(--foreground))]">
          ...
          <button onClick={handleEncode} className="w-full py-2.5 bg-violet-600 text-white rounded-lg font-medium text-sm hover:opacity-90 active:scale-[0.97] transition-all">合成 NCI</button>
          {encodeResult ? (
            encodeResult.error ? <ResultBox empty><ResultRow label="错误" value={<span className="text-red-500">{encodeResult.error}</span>} /></ResultBox> : (
              <ResultBox>
      ...
      <div className="p-4 rounded-xl bg-violet-50 dark:bg-violet-900/20 border border-violet-200 dark:border-violet-800 text-xs text-violet-800 dark:text-violet-300 leading-relaxed">
        <strong>NCI 结构（3GPP TS 38.413）：</strong><br />
...
function RefTab() {
  ...
  const lteColumns = [
    ...
    { key: 'mode', title: '模式', width: 80, render: (v: string) => <Badge color={v === 'FDD' ? 'blue' : 'green'}>{v}</Badge> },
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-base font-semibold text-[hsl(var(--foreground))] mb-3 flex items-center gap-2"><Signal className="w-4 h-4 text-blue-600" />LTE 频段 EARFCN 参考表</h3>
